  ...
  const Field = ({ label, value }: { label: string; value: any }) => {
    ...
    return (
      <div className="group border-b border-slate-50 py-5 last:border-0">
        <dt className="mb-2 text-xs font-medium leading-relaxed text-slate-400 transition-colors group-hover:text-slate-500">
          {label}
        </dt>
        <dd className="text-sm leading-relaxed text-slate-900">
          {Array.isArray(value) ? (
            ...
            <div className="whitespace-pre-wrap rounded-2xl bg-slate-50/50 p-4 text-slate-800 border border-slate-100/50">
              {value}
            </div>
          ) : (
            <span className="font-bold text-base">{displayValue(value)}</span>
          )}
        </dd>
      </div>
    );
    ...
    return (
      <section id={id} className="mb-12 scroll-mt-24">
        <h2 className="mb-6 flex items-center gap-3 text-lg font-bold text-slate-900">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-600 text-[10px] font-black text-white shadow-lg shadow-blue-100">
            {title.split(".")[0]}
          </span>
          {title.split(".")[1].trim()}
        </h2>
        ...
        <nav className="sticky top-24 space-y-1">
          ...
          {SECTIONS.map((section) => (
            <button
              ...
              className={cn(
                "w-full px-4 py-2.5 text-left text-xs font-bold transition-all rounded-xl",
      ...
      <div className="flex-1">
        <Section id="contact" title="1. Контактна інформація">
          ...
          {data.stakeholders && (data.stakeholders as any[]).length > 0 && (
            <div className="py-5">
              <dt className="mb-3 text-xs font-medium text-slate-400">1.5 Чи є інші учасники проєкту?</dt>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {(data.stakeholders as any[]).map((s, i) => (
                  <div key={i} className="rounded-2xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:border-slate-200">
                    <p className="text-sm font-black text-slate-900">{s.name}</p>
                    <p className="mt-1 text-xs font-medium text-slate-500">{s.role} • {s.contact}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <Section id="flow" title="8. Шлях користувача">
          ...
          <Field label="Інші способи знайомства" value={data.onboardingOther} />
        </Section>
        ...
        <Section id="implementation" title="13. Терміни та умови">
          ...
          <Field label="Уточнення щодо хостингу" value={data.hostingNote} />
